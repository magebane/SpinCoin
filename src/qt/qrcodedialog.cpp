...
{
    QString ret = QString("spincoin:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non SPC unit input in lnReqAmount, we generate the URI with SPC as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(spincoinUnits::format(spincoinUnits::SPC, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
